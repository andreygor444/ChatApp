...
$(document).ready(function() {
    const resetAvatarBtn = $("#reset-new-user-avatar-btn")
    const userAvatar = $("#new-user-avatar-img")
    const userAvatarOffset = userAvatar.offset()
    resetAvatarBtn.css({
        "position": "fixed",
        "top": userAvatarOffset.top+20+"px",
        "left": userAvatarOffset.left+160+"px"
    })
    if (userAvatar.attr("src") !== "/static/img/user_avatars/default/icon.png") {
        resetAvatarBtn.css({"visibility": "visible"})
    }
    resetAvatarBtn.click(resetNewChatAvatar)
