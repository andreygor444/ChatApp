...
def profile():
    ...
    if request.method == "GET":
        ...
        invite_link = f"http://{HOST}:{PORT}/register/{unique_codes_manager.get_unique_code()}"
        return render_template("profile.html", user=current_user, notifications=all_notifications,
                               random_number=randrange(1, 500), invite_link=invite_link)
...
def get_chat_messages_handler(chat_id):
    ...
    return jsonify(
        [member.to_dict(only=("id", "name", "surname")) for member in members if member.id != current_user.id])
